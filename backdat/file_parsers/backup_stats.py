...
def update_stats(completed_action, action_status, filepath=DEFAULT_STATS_PATH):
    ...
    if action_status == ACTION_STATUS.DONE:
        stats[KEYS.ACTIONS_COMPLETED] = stats.get(KEYS.ACTIONS_COMPLETED, 0) + 1
        ...
        stats[KEYS.BYTES_UPLOADED] = (
            stats.get(KEYS.BYTES_UPLOADED, 0) +
            os.stat(completed_action.source).st_size
        )
    elif action_status == ACTION_STATUS.FAIL:
        stats[KEYS.ACTIONS_FAILED] = stats.get(KEYS.ACTIONS_FAILED, 0) + 1
    else:

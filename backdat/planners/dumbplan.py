...
def make_plan():
    ...
    # backups, and namespaced by hostname to prevent filepath collisions.
    # You may not want this, so an option to configure it
    ...
    with open("/var/opt/backdat/backup-plan.tsv", 'w') as planfile:
        ...
        for fstat in get_fileset_statlist():   # for every file in fileset
            ...
            trg_dir = '/'.join(fstat[STAT_KEYS.SOURCE].split('/')[:-2])
